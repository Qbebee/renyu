...
cursor.execute("select count(*)from gondan_9m where fadriq = '%s'"%fq)
db.commit()
...
if persons >1 :
    for person in range(persons):
        ...
        sqlstr="insert into mydb.gondan_9m(fadriq,name,bans,sanj,bans2,sanj2,totals) values('%s','%s','%d','%d','%d','%s','%d')"%(fq,name,bans,sanj,bans2,sanj2,int(int(bans)*198+int(sanj)))
...
#查验一下当天录入的数据是否有重复或漏记
cursor.execute("select * from gondan_9m where fadriq ='%s'"%fq)
db.commit()
print("")
...
#cursor.close()   
print('')
print('9月份翻堆组个人累计数据汇总如下：')
logger.log(logging.INFO,"9月份翻堆组个人累计数据汇总如下：")
cursor.execute('select name ,count(name),sum(bans),sum(sanj),sum(totals) from gondan_9m group by name order by fadriq')
db.commit()
